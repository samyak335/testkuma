...
class QueryCompiler_SQLite3 extends QueryCompiler {
    ...
    insert() {
        ...
        const { onConflict, ignore, merge } = this.single;
        if (onConflict && ignore)
            sql += ' where true' + this._ignore(onConflict);
        else if (onConflict && merge) {
            sql +=
    ...
    _merge(updates, columns, insert) {
        ...
        if (updates && Array.isArray(updates)) {
            sql += updates
                .map((column) => wrapString(column.split('.').pop(), this.formatter.builder, this.client, this.formatter))
