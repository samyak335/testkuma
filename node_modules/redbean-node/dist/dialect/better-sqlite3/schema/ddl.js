...
const { createNewTable, copyAllData, dropOriginal, copyData, renameTable, getTableSql, } = require('./internal/sqlite-ddl-operations');
const { parseCreateTable, parseCreateIndex } = require('./internal/parser');
const { compileCreateTable, compileCreateIndex, } = require('./internal/compiler');
class SQLite3_DDL {
        ...
        return this.client.transaction((trx) => {
            ...
            return Promise.all(columns.map((column) => this.getColumn(column)))
                ...
                for (const createIndex of createIndices) {
                    ...
                    parsedIndex.columns = parsedIndex.columns.filter((newColumn) => newColumn.expression ||
                        !columns.some((column) => newColumn.name === this.formatter(column)));
    ...
    async dropForeign(columns, foreignKeyName) {
        return this.client.transaction(async (trx) => {
            ...
            parsedTable.constraints = parsedTable.constraints.filter((constraint) => {
                ...
                return (constraint.columns.some((column) => columns.includes(column)) ===
                    false);
            });
            const newTable = compileCreateTable(parsedTable, this.wrap);
            return this.alter(newTable, createIndices, (row) => {
    ...
    async dropPrimary(constraintName) {
        return this.client.transaction(async (trx) => {
            ...
            const updatedDefs = defs
                ...
                .map((line) => line.trim())
                .filter((defLine) => {
                if (defLine.startsWith('constraint') === false &&
                    defLine.includes('primary key') === false)
                    return true;
                if (constraintName) {
                    if (defLine.includes(constraintName))
                        return false;
                    return true;
                }
                else {
